...
class BBDataset(Dataset):
    ...
    def cache_to_memory(self):
        def mof_criterion(mof):
            ...
            return mof.num_components <= self.max_bbs
        ...
        def bb_criterion(bb):
            ...
            #     print("bond_dist.numel:", bond_dist.numel())
        ...
        for i in tqdm(self.samples):
            try:
                data = self[i]
                if not mof_criterion(data):
                    print("mof_criterion(data):", mof_criterion(data))
    ...
    def cache_to_memory(self):
        ...
        for i in tqdm(self.samples, total=self.num_samples):
            try:
                ...
                if mof_criterion(data):
                    bb_all_success = True
                    # print("len of bbs", len(data.bbs))
                    for bb in data.bbs:
                        cart_coords, success = bb_criterion(bb)
                        if success:
                            # print("cart_coords:", cart_coords)
                            bb.num_nodes = bb.num_atoms
                            if cart_coords.size(0) < 2:
                                bb.diameter = torch.tensor(0.0)  # 或者设置为其他适当的值
                            else:
                                bb.diameter = torch.pdist(cart_coords).max()
                            if "fp" not in bb:
                                bb.fp = get_bb_fingerprint(bb).view(1, -1)
                    ...
                    if bb_all_success:
                        ...
                        # print("data frac_coords:", data.frac_coords)
...
#         def bb_criterion(bb):
#             # print("num_atoms:", bb.num_atoms)
#             # print("is_anchor:", bb.is_anchor)
#             bb.num_cps = bb.is_anchor.long().sum()
#             # print("num_cps:", bb.num_cps)
#             if (bb.num_atoms > self.max_atoms) or (bb.num_cps > self.max_cps):
#                 return None, False
...
#                 success = False
#             else:
#                 condition1 = pdist.min() > 0.25
#                 condition2 = bond_dist.max() < 5.0
#                 condition3 = bb.num_atoms <= self.max_atoms
#                 condition4 = bb.num_cps <= self.max_cps
#                 # if not condition1:
#                 #     print(f"Condition 1 (pdist.min() > 0.25): {condition1}")
#                 # if not condition2:
#                 #     print(f"Condition 2 (bond_dist.max() < 5.0): {condition2}")
#                 # if not condition3:
#                 #     print(f"Condition 3 (bb.num_atoms <= self.max_atoms): {condition3}")
#                 # if not condition4:
#                 #     print(f"Condition 4 (bb.num_cps <= self.max_cps): {condition4}")
...
#             ).to(self.device)
#             # print(batch)
#             # print("before transform:", batch.atom_types)
#             batch.atom_types = self.bb_encoder.type_mapper.transform(batch.atom_types)
#             # print("after transform:", batch.atom_types)
#             with torch.no_grad():
#                 # print(batch)
#                 bb_emb = self.bb_encoder.encode(batch)
#                 # print("bb_emb:", bb_emb)
#                 # print("bb shape:", bb_emb.shape)
#             all_bb_emb.append(bb_emb.cpu())
#             print("in cpu()")
#         all_bb_emb = torch.cat(all_bb_emb, dim=0)
#         all_bb_emb = torch.clamp(
#             all_bb_emb, -self.bb_emb_clipping, self.bb_emb_clipping
#         )
#         print("building block embeddings shape:", all_bb_emb.shape)
#         # save building block lookup table
#         if hasattr(self, "bb_cache_path"):
#             torch.save([all_bbs, all_bb_emb], self.bb_cache_path)
...
#         for i in range(len(self.cached_data)):
#             self.cached_data[i].atom_types = all_bb_emb[offsets[i] : offsets[i + 1]]
#         print("embedded building block embeddings.")

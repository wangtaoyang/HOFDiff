...
class MOFDiff(pl.LightningModule):
    ...
    def encode(self, batch):
        ...
        # print(batch)
        # print()
        # print("node_embeddings:", batch.node_embedding)
    ...
    def forward(self, batch, teacher_forcing, t_int=None):
        ...
        n_metal_loss = 0.0
        # print("pred_num_atoms:", pred_num_atoms)
        # print("pred_lengths_and_angles:", pred_lengths_and_angles)
        # print("mu:", mu)
    ...
    def sample(
        ...
        for t in tqdm(reversed(range(1, self.T)), disable=disable_bar):
            ...
            # 假如x,h 不为nan，则输出，否则输出报错信息和迭代次数
            if torch.isnan(x).any() or torch.isnan(h).any():
                i = self.T - t[0]
                print(f"x,h, 在第{i}次迭代")
                print("x:",x)
                print("h:",h)
                print("frac_x:", frac_x)
    ...
    def configure_optimizers(self):
        ...
        # }
        return { "optimizer": opt,  "lr_scheduler": {  "scheduler": scheduler,  "strict": False, "monitor": "val_diffusion_loss" }, }
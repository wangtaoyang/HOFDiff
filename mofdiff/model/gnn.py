...
class GemNetOCEncoder(nn.Module):
    ...
    def forward(self, data, no_emb=False):
        # print("frac_coords shape:", data.frac_coords.shape)
        # print("lengths shape:", data.lengths.shape)
        # print("angles shape:", data.angles.shape)
        # print("num_atoms:", data.num_atoms.shape)
        ...
        # print("out:", out)
        # 假如out中有nan值，打印data中的信息
        # if torch.isnan(out).any():
            # print("nan in out")
            # print("data.pos:", data.pos)
            # print("data.natoms:", data.natoms)
            # print("data.batch:", data.batch)
            # print("data.num_atoms:", data.num_atoms)
            # print("data.atom_types:", data.atom_types)
            # print("data.edge_index:", data.edge_index)
            # print("data.edge_attr:", data.edge_attr)
            # print("data.frac_coords:", data.frac_coords)
            # print("data.lengths:", data.lengths)
            # print("data.angles:", data.angles)
            # print("data.diameter:", data.diameter)
            # print("data.num_cps:", data.num_cps)
            # print("data.is_anchor:", data.is_anchor)
            # exit(0)
        return out

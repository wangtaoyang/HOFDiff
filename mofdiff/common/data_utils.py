...
def lattice_params_to_matrix_torch(lengths, angles):
    ...
    vector_b = torch.stack([b * torch.cos(gamma), b * torch.sin(gamma), torch.zeros_like(b)], dim=1)
    vector_c = torch.stack([
        c * torch.cos(beta),
        c * (torch.cos(alpha) - torch.cos(gamma) * torch.cos(beta) / torch.sin(gamma)),
        c * torch.sin(alpha)
    ], dim=1)
...
def radius_graph_pbc(
    ...
    unit_cell = SUPERCELLS.clone().detach().float().to(device)

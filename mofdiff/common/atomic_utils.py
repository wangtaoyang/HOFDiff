...
def cal_cif_combined(cif1, cif2, Hbond=False):
    ...
    # print(f"combine {cif1_name} {cif2_name}")
    # print("cif1_bond_len:", cif1_bond_len)
    # print("cif2_bond_len:", cif2_bond_len)
    # print("combined_bond_len:", combined_bond_len)
...
def extract_atom_labels(data_block):
    ...
    for line in data_block:
        ...
        if len(parts) < 2:
            continue
        atom_labels.append(parts[0].replace('?', ''))  # 假设原子标签总是在第二列
    return atom_labels
...
def compute_bonds(distance_mat, atomic_numbers, Hbond = False, cif = None):
    ...
    for i, e1 in enumerate(allatomtypes[:-1]):
        for j, e2 in enumerate(allatomtypes[i + 1 :]):
            ...
            # if Hbond:
            # # Handling hydrogen bonds explicitly
            #     if 'H' in elements and (e1 in ['N', 'O', 'F'] or e2 in ['N', 'O', 'F']):
            #         if 1.5 <= dist <= 2.5:  # Typical range for hydrogen bonds
            #             all_bonds.append([i, i + j + 1])
            #             continue
...
# pyg_graph_from_cif('/opt/wty/hof_synthesis/HOF/MOFDiff/mofdiff/data/hof_data/48_1846107_19/48_1846107_19.cif', Hbond=True)
...
def assemble_local_struct(building_blocks, mof_asr=None, device='cpu'):
    ...
    for mol_idx, mol in enumerate(data.pyg_mols):
        ...
        # print("before unique num_anchors:", anchor_shifted_frac_coords.shape[0])
        anchor_shifted_frac_coords, inverse_index = torch.unique(
            anchor_shifted_frac_coords, dim=0, return_inverse=True
        )
        num_anchors = anchor_shifted_frac_coords.shape[0]
        # print("after unique num_anchors:", num_anchors)

        if len(docker_frac_coords) == 0:
            # print("len docker_frac_coords == 0")
            # 检查mol中是否有frac_coords且数量大于0以避免在空数据上执行mean

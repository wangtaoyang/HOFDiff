...
def load_bb_encoder(model_path):
    print("in load_bb_encoder: ", model_path)
    cfg = OmegaConf.load(os.path.join(model_path, "hparams.yaml"))    
    cfg.model.encoder.scale_file = str(
        Path(PROJECT_ROOT) / "hofdiff/model/gemnet_oc/gemnet-oc.pt"
    )
...
def load_hofdiff_model(model_path, config_overrides=None, load_last=False):

...
from hofdiff.common.atomic_utils import cal_cif_combined
...
def process_folders(bbs_path):
    for sub_folder in os.listdir(bbs_path):
        folder_path = os.path.join(bbs_path, sub_folder)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Root path where building block CIF files are located",
    )
    args = parser.parse_args()

    # Call the processing function with the provided root path
    process_folders(args.bbs_path)

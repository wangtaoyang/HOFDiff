...
from hofdiff.common.sys_utils import PROJECT_ROOT
from hofdiff.model.utils import build_mlp
from hofdiff.model.gemnet_oc.layers.radial_basis import RadialBasis
...
class BBEncoder(pl.LightningModule):
    ...
    def forward(self, batch):
        ...
        if torch.isnan(batch.edge_index).any():
            print("nan in batch.edge_index")
        if torch.isnan(batch.frac_coords).any():
            print("nan in batch.frac_coords")
        if torch.isnan(batch.atom_types).any():
            print("nan in batch.atom_types")
        if torch.isnan(batch.diameter).any():
            print("nan in batch.diameter")
        if torch.isnan(batch.num_atoms).any():
            print("nan in batch.num_atoms") 
        if torch.isnan(batch.num_cps).any():
            print("nan in batch.num_cps")
        if torch.isnan(batch.is_anchor).any():
            print("nan in batch.is_anchor")
        if torch.isnan(batch.angles).any():
            print("nan in batch.angles")
        z = self.encode(batch)
        # print("z:", z)
